...
test('User can book homevisitdoctor service end-to-end', async ({ page }) => {
  ...
  await dashboard.selectService('Home visit Doctor');

  // 2️⃣ Packages / Procedures page
  const packagesPage = new PackagesPage(page);
  await packagesPage.verifyHomeVisitDoctorLoaded();

  // ✅ Home visit doctor package selection
  await packagesPage.selectHomeVisitDoctor();
