...
test('User can book Vitamin IV service end-to-end', async ({ page }) => {
  ...
  // ✅ Select Vitamin IV
  await dashboard.selectService('Vitamin IV Drips');

  // 2️⃣ Packages / Procedures page
  const packagesPage = new PackagesPage(page);
  await packagesPage.verifyVitaminIVLoaded();

  // ✅ Select Vitamin IV procedure
  await packagesPage.selectVitaminIVPackage('Hair Health IV Therapy');
  ...
  await patientPage.fillHealthInfo();
  await patientPage.clickContinueButton();
